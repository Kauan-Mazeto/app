...
const toPatient = (p) => ({
  id: p.id, name: p.name, cpf: p.cpf, birth_date: p.birthDate,
  phone: p.phone, address: p.address, lgpd_accepted: p.lgpdAccepted,
  missed_count: p.missedCount, blocked_online: p.blockedOnline,
});
const toAppt = (a) => ({
  id: a.id, patient_id: a.patientId, doctor_id: a.doctorId,
  specialty: a.specialty, priority: a.priority, unit: a.unit, status: a.status,
...
api.post("/auth/login", async (req, res) => {
  ...
  res.cookie("access_token", token, { httpOnly: true, sameSite: "lax", maxAge: 86400_000 });
  const { passwordHash, ...safe } = user; res.json({ ...safe, token });
});
api.post("/auth/logout", (req, res) => { res.clearCookie("access_token"); res.json({ ok: true }); });
api.get("/auth/me", requireAuth, (req, res) => { const { passwordHash, ...s } = req.user; res.json(s); });

api.get("/users", requireAuth, async (req, res) => {
  const r = req.user.role;
  if (!["admin", "secretario", "atendente"].includes(r)) return res.status(403).json({ detail: "Acesso negado" });
...
api.post("/users", requireAuth, requireRoles("admin"), async (req, res) => {
  ...
  const u = await prisma.user.create({ data: { email: email.toLowerCase(), passwordHash: await bcrypt.hash(password, 10), name, role, crm, specialty, unit } });
  await audit(req.user, "user.create", u.id, { role });
  const { passwordHash, ...safe } = u; res.json(safe);
});

api.get("/patients", requireAuth, async (req, res) => {
  const q = req.query.q;
  const where = q ? { OR: [{ name: { contains: q } }, { cpf: { contains: q } }] } : {};
...
api.post("/patients", requireAuth, requireRoles("atendente", "admin", "medico"), async (req, res) => {
  try {
    const p = await prisma.patient.create({ data: {
      name: req.body.name, cpf: req.body.cpf, birthDate: req.body.birth_date,
      phone: req.body.phone, address: req.body.address, lgpdAccepted: !!req.body.lgpd_accepted,
    }});
    res.json(toPatient(p));
  } catch { res.status(400).json({ detail: "CPF já cadastrado" }); }
});

api.get("/appointments", requireAuth, async (req, res) => {
  const where = {};
  if (req.query.date) {
    const d = new Date(req.query.date + "T00:00:00"); const end = new Date(d); end.setDate(end.getDate() + 1);
  ...
  const appts = await prisma.appointment.findMany({ where, include: { patient: true }, orderBy: { scheduledAt: "asc" } });
  res.json(appts.map(toAppt));
});
api.post("/appointments", requireAuth, requireRoles("atendente", "admin"), async (req, res) => {
  const a = await prisma.appointment.create({ data: {
    patientId: req.body.patient_id, doctorId: req.body.doctor_id,
    specialty: req.body.specialty, scheduledAt: new Date(req.body.scheduled_at),
    priority: req.body.priority || "normal", unit: req.body.unit || "UBS Central",
  }});
  res.json(toAppt(a));
});
api.patch("/appointments/:id", requireAuth, async (req, res) => {
  ...
  await prisma.appointment.update({ where: { id: a.id }, data: { status: req.body.status, justification: req.body.justification } });
  if (req.body.status === "faltou" && !req.body.justification) {
    const p = await prisma.patient.update({ where: { id: a.patientId }, data: { missedCount: { increment: 1 } } });
    if (p.missedCount >= 2) await prisma.patient.update({ where: { id: p.id }, data: { blockedOnline: true } });
  }
...
api.post("/prescriptions", requireAuth, requireRoles("medico"), async (req, res) => {
  ...
  const existing = await prisma.prescription.findFirst({ where: { patientId: patient_id, activeSubstance: active_substance, active: true } });
  if (existing && !justification)
    return res.status(409).json({ detail: { error: "conflict", message: `Paciente já possui receita ativa para ${active_substance}. Selecione uma justificativa.` } });
  if (existing && justification)
    await prisma.prescription.update({ where: { id: existing.id }, data: { active: false } });
  const p = await prisma.prescription.create({ data: {
    patientId: patient_id, doctorId: req.user.id, doctorName: req.user.name, doctorCrm: req.user.crm,
    medication: req.body.medication, activeSubstance: active_substance,
    dosage: req.body.dosage, frequency: req.body.frequency, durationDays: req.body.duration_days,
    route: req.body.route || "Oral", schedule: JSON.stringify(req.body.schedule || []),
  ...
  res.json({ ...p, validation_code: p.validationCode, active_substance: p.activeSubstance });
});
...
api.patch("/exams/:id/status", requireAuth, requireRoles("atendente", "admin"), async (req, res) => {
  ...
  if (!["pendente", "pronto", "retirado"].includes(status)) return res.status(400).json({ detail: "Status inválido" });
  const data = { status };
  if (status === "retirado") { data.deliveredAt = new Date(); data.deliveredById = req.user.id; }
...
api.get("/waiting-list", requireAuth, async (req, res) => {
  ...
  const wl = await prisma.waitingList.findMany({ where, include: { patient: true }, orderBy: { createdAt: "asc" } });
  res.json(wl.map(w => ({ id: w.id, patient_id: w.patientId, specialty: w.specialty, patient: { name: w.patient.name } })));
});
api.get("/vacancies/active", requireAuth, async (req, res) => {
  const now = new Date();
  const vacs = await prisma.vacancy.findMany({ where: { status: { in: ["notified", "waiting_response"] } } });
  const out = [];
  for (const v of vacs) {
    const remaining = Math.max(0, Math.floor((v.deadline - now) / 1000));
    if (remaining <= 0 && v.status === "waiting_response") {
      await prisma.vacancy.update({ where: { id: v.id }, data: { status: "expired" } });
    } else out.push({
      id: v.id, patient_id: v.patientId, patient_name: v.patientName,
      specialty: v.specialty, unit: v.unit, deadline: v.deadline.toISOString(),
      remaining_seconds: remaining, status: v.status,
    });
  }
  res.json(out);
});
...
api.get("/dashboard/secretario", requireAuth, requireRoles("secretario", "admin"), async (req, res) => {
  ...
  for (const a of all) {
    ...
    bySpec[a.specialty].total++; if (a.status === "faltou") bySpec[a.specialty].faltas++;
    byUnit[a.unit] ??= { total: 0, faltas: 0 };
    byUnit[a.unit].total++; if (a.status === "faltou") byUnit[a.unit].faltas++;
  }
  const prescs = await prisma.prescription.findMany({ where: { active: true } });
  const medMap = {}; for (const p of prescs) medMap[p.medication] = (medMap[p.medication] || 0) + 1;
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const weekly = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date(today); d.setDate(d.getDate() - i);
    const nd = new Date(d); nd.setDate(nd.getDate() + 1);
    const dayAppts = all.filter(a => a.scheduledAt >= d && a.scheduledAt < nd);
    const f = dayAppts.filter(a => a.status === "faltou").length;
    weekly.push({ date: d.toISOString().slice(0, 10), total: dayAppts.length, faltas: f, compareceu: dayAppts.length - f });
  }
  const scores = Array.from({ length: 50 }, () => 6 + Math.floor(Math.random() * 5));
  const nps = Math.round((scores.filter(s => s >= 9).length - scores.filter(s => s <= 6).length) / scores.length * 100);
  res.json({
    kpis: {
    ...
    by_specialty: Object.entries(bySpec).map(([s, v]) => ({ specialty: s, total: v.total, faltas: v.faltas, absenteeism: v.total ? +(v.faltas / v.total * 100).toFixed(1) : 0 })),
...
api.get("/audit-logs", requireAuth, requireRoles("secretario", "admin"), async (req, res) => {
  ...
  res.json(logs.map(l => ({ id: l.id, action: l.action, target: l.target, user_name: l.userName, user_role: l.userRole, timestamp: l.timestamp.toISOString(), details: JSON.parse(l.details) })));
});

const CID = [{code:"I10",desc:"Hipertensão essencial"},{code:"E11",desc:"Diabetes tipo 2"},{code:"F32",desc:"Episódios depressivos"},{code:"F41",desc:"Transtornos ansiosos"},{code:"J45",desc:"Asma"},{code:"K21",desc:"Refluxo gastroesofágico"},{code:"M54",desc:"Dorsalgia"}];
const TUSS = [{code:"10101012",desc:"Consulta em consultório"},{code:"10103011",desc:"Consulta pré-natal"},{code:"40202100",desc:"Hemograma completo"},{code:"40301974",desc:"Glicemia de jejum"},{code:"40304035",desc:"Colesterol total"}];
const SIGTAP = [{code:"02.02.01.038-0",desc:"Hemograma completo"},{code:"02.02.01.014-9",desc:"Glicemia em jejum"},{code:"02.02.01.019-0",desc:"Colesterol total"},{code:"02.02.02.006-3",desc:"Urina - EAS"},{code:"02.05.02.014-4",desc:"Eletrocardiograma"},{code:"02.05.01.004-1",desc:"Radiografia de tórax"},{code:"02.11.06.008-8",desc:"Ultrassonografia abdominal"}];
const search = (arr, q) => !q ? arr : arr.filter(c => c.code.toLowerCase().includes(q.toLowerCase()) || c.desc.toLowerCase().includes(q.toLowerCase()));
