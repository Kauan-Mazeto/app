...
async function seedIA() {
  const medicos = await prisma.user.findMany({ where: { role: "medico" } });
  const patients = await prisma.patient.findMany();
  if (medicos.length === 0 || patients.length === 0) { console.log("Rode o seed principal primeiro."); return; }

  const hoje = new Date();
  let criadas = 0;

  const especialidades = [
    { specialty: "Clínica Geral", mesesPico: [0, 1, 6, 7] },
    { specialty: "Cardiologia", mesesPico: [5, 6, 7] },
  ];

  for (let mesAtras = 11; mesAtras >= 0; mesAtras--) {
    const dataBase = new Date(hoje);
    dataBase.setMonth(dataBase.getMonth() - mesAtras);
    const mes = dataBase.getMonth();

    for (const esp of especialidades) {
      const isPico = esp.mesesPico.includes(mes);
      const qtd = isPico ? 18 : 5;

      for (let i = 0; i < qtd; i++) {
        const dia = new Date(dataBase);
        dia.setDate(1 + (i % 25));
        dia.setHours(8 + (i % 8), 0, 0, 0);
        const medico = medicos.find(m => m.specialty === esp.specialty) || medicos[0];
        const paciente = patients[i % patients.length];
        await prisma.appointment.create({
          data: {
            patientId: paciente.id, doctorId: medico.id, specialty: esp.specialty,
            scheduledAt: dia, priority: i % 5 === 0 ? "urgente" : i % 3 === 0 ? "preferencial" : "normal",
            unit: medico.unit || "UBS Central", status: "compareceu",
  ...
  const tiposExame = ["Hemograma completo", "Glicemia em jejum", "Colesterol total", "Eletrocardiograma", "Ultrassonografia abdominal"];
  for (let mesAtras = 5; mesAtras >= 0; mesAtras--) {
    const dataBase = new Date(hoje);
    dataBase.setMonth(dataBase.getMonth() - mesAtras);
    const mes = dataBase.getMonth();
    const isVerao = [11, 0, 1, 2].includes(mes);
    for (let i = 0; i < 10; i++) {
      const dia = new Date(dataBase); dia.setDate(1 + (i % 25));
      await prisma.exam.create({
        data: {
          patientId: patients[i % patients.length].id, exam: tiposExame[i % tiposExame.length],
          status: isVerao ? (i % 3 === 0 ? "pronto" : "retirado") : "retirado",
          requestedById: medicos[0].id, createdAt: dia,
        },
      });
    }
  }

  const medicamentosInverno = [["Amoxicilina 500mg", "Amoxicilina"], ["Azitromicina 500mg", "Azitromicina"], ["Ibuprofeno 600mg", "Ibuprofeno"]];
  const medicamentosVerao = [["Dipirona 500mg", "Dipirona"], ["Soro de Reidratação Oral", "Sais de Reidratação"], ["Loratadina 10mg", "Loratadina"]];

  for (let mesAtras = 11; mesAtras >= 0; mesAtras--) {
    const dataBase = new Date(hoje);
    dataBase.setMonth(dataBase.getMonth() - mesAtras);
    const mes = dataBase.getMonth();
    const isInverno = [5, 6, 7].includes(mes);
    const isVerao = [11, 0, 1].includes(mes);
    const listaMeds = isInverno ? medicamentosInverno : isVerao ? medicamentosVerao : [];
    for (const med of listaMeds) {
      for (let i = 0; i < 4; i++) {
        const medico = medicos[i % medicos.length];
        const paciente = patients[(i + mesAtras) % patients.length];
        await prisma.prescription.create({
          data: {
            patientId: paciente.id, doctorId: medico.id, doctorName: medico.name, doctorCrm: medico.crm,
            medication: med[0], activeSubstance: med[1], dosage: "1 comprimido", frequency: "3x ao dia",
            durationDays: 7, route: "Oral", schedule: JSON.stringify(["08:00", "14:00", "20:00"]),
            validationCode: `GOVBR-${randomUUID().replace(/-/g, "").slice(0, 12).toUpperCase()}`,
            active: mesAtras > 0 ? false : true, createdAt: dataBase,
          },
        });
      }
    }
  ...
  console.log(`Seed IA completo. ${criadas} consultas criadas com padroes sazonais.`);
}
