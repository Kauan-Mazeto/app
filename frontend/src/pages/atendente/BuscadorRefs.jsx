  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="font-display text-4xl font-extrabold text-[#1D3557] tracking-tight">Buscador Técnico</h1>
        <p className="text-slate-500 mt-1">CID-10 · TUSS · SIGTAP — códigos padronizados do SUS/ANS.</p>
      </div>
      ...
      <div className="sc-card">
        <div className="relative mb-4">
          ...
          <input data-testid="ref-search" value={q} onChange={(e) => setQ(e.target.value)}
        ...
        <div className="space-y-1">
          {results.map(r => (
            <div key={r.code} className="flex items-start gap-4 py-2 border-b border-slate-100 last:border-0">
              <div className="font-mono-nums text-xs bg-slate-100 px-2 py-1 rounded font-bold text-[#1D3557]">{r.code}</div>
              <div className="text-sm text-slate-700">{r.desc}</div>
            </div>
          ))}
          {results.length === 0 && <div className="text-sm text-slate-400 text-center py-6">Nenhum código encontrado.</div>}
        </div>
      </div>
    </div>
  );
