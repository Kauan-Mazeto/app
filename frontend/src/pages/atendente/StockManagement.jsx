...
import { ArrowDownCircle, ArrowUpCircle, Boxes, Download, Search } from "lucide-react";
...
export default function StockManagement({ initialTab = "entry", mode = "atendente" }) {
  const { user } = useAuth();
  const isSecretaryView = user?.role === "secretario" || mode === "secretario";
  const [activeTab, setActiveTab] = useState(isSecretaryView ? "stock" : initialTab);
  const [entryForm, setEntryForm] = useState(defaultForm());
  const [exitForm, setExitForm] = useState(defaultForm());
  const [units, setUnits] = useState([]);
  const [stockItems, setStockItems] = useState([]);
  const [movements, setMovements] = useState([]);
  const [stockFilter, setStockFilter] = useState({ medicine: "", unit: "", unitId: "" });
  ...
  useEffect(() => {
    try {
      const savedEntry = JSON.parse(window.localStorage.getItem(ENTRY_STORAGE_KEY) || "null");
      const savedExit = JSON.parse(window.localStorage.getItem(EXIT_STORAGE_KEY) || "null");
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (availableUnits.length && !entryForm.selectedUnitId) {
        const preferred = availableUnits.find((unit) => unit.id === user?.healthUnitId) || availableUnits[0];
        setEntryForm((prev) => ({ ...prev, selectedUnitId: preferred?.id || "" }));
      }
      if (availableUnits.length && !exitForm.selectedUnitId) {
        const preferred = availableUnits.find((unit) => unit.id === user?.healthUnitId) || availableUnits[0];
        setExitForm((prev) => ({ ...prev, selectedUnitId: preferred?.id || "" }));
      }
    } catch (e) {
      console.error(e);
  ...
  const filteredStock = useMemo(() => {
    ...
    return stockItems.filter((item) => {
      const matchesMedicine = !medicineTerm || [item.medicineId, item.medicineName].join(" ").toLowerCase().includes(medicineTerm);
      const matchesUnit = !stockFilter.unitId && !unitTerm
        ? true
        : stockFilter.unitId
  ...
  const filteredMovements = useMemo(() => {
    return movements.filter((item) => {
      ...
      if (historyFilterMode === "type") {
        if (!term) return true;
        const selected = term === "entrada" ? "ENTRY" : term === "saida" ? "EXIT" : "";
      ...
      if (!term) return true;
      return [item.medicineName, item.unit, item.user?.name, item.type].join(" ").toLowerCase().includes(term);
    });
  }, [movements, historyFilterMode, historyFilterValue, historyDateFrom, historyDateTo]);

  const updateEntryForm = (changes) => setEntryForm((prev) => ({ ...prev, ...changes }));
  const updateExitForm = (changes) => setExitForm((prev) => ({ ...prev, ...changes }));
  ...
  const exportMovementsCsv = () => {
    ...
    const headers = ["dataHora", "tipo", "medicamento", "unidade", "usuario", "quantidade"];
  ...
  const handleHistoryDateFromChange = (value) => {
    ...
    if (value && historyDateTo && value > historyDateTo) {
      ...
      toast.warning("A data final foi ajustada para a data inicial, pois não pode ser anterior.");
    }
  };
  ...
  const submitEntry = async () => {
    ...
    try {
      ...
      setEntryForm({ ...defaultForm(), selectedUnitId: entryForm.selectedUnitId });
      await loadData();
      setActiveTab("stock");
    } catch (e) {
  ...
  const submitExit = async () => {
    ...
    try {
      ...
      setExitForm({ ...defaultForm(), selectedUnitId: exitForm.selectedUnitId });
      await loadData();
      setActiveTab("stock");
    } catch (e) {
  ...
  const renderForm = (kind, form, updateForm, title, description, submitLabel, submitAction, buttonTone) => (
    <div className="grid grid-cols-1 gap-4">
      ...
      <select value={form.selectedUnitId} onChange={(e) => updateForm({ selectedUnitId: e.target.value })} className="inp">
      ...
      <input value={form.medicine} onChange={(e) => updateForm({ medicine: e.target.value })} className="inp" placeholder="Nome ou código do medicamento" />
      <label className="text-sm">Dosagem</label>
      <input value={form.dosage} onChange={(e) => updateForm({ dosage: e.target.value })} className="inp" placeholder="Ex.: 500mg" />
      <label className="text-sm">Lote / Referência</label>
      <input value={form.lot} onChange={(e) => updateForm({ lot: e.target.value })} className="inp" placeholder="Opcional" />
      <label className="text-sm">Observações</label>
      <textarea value={form.notes} onChange={(e) => updateForm({ notes: e.target.value })} className="inp min-h-[90px]" placeholder="Informações adicionais do medicamento" />
      <label className="text-sm">Quantidade</label>
      <input type="number" min={1} value={form.quantity} onChange={(e) => updateForm({ quantity: Number(e.target.value) })} className="inp" />
      <div className="flex justify-end">
        <button onClick={submitAction} disabled={submitting} className={buttonTone}>
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        <div className="overline text-[#457B9D]">{isSecretaryView ? "Estoque · Visualização" : "Estoque · Gestão"}</div>
        <h1 className="font-display text-3xl font-extrabold text-[#1D3557]">
          {isSecretaryView ? "Visualização de estoque" : "Controle de entradas, saídas e saldo"}
        </h1>
          ...
          <div className="sc-card p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h2 className="text-xl font-bold text-[#1D3557]">Estoque por medicamento e unidade</h2>
              </div>
              <div className="flex flex-col gap-3 md:flex-row md:items-center">
                <div className="relative">
                  <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input value={stockFilter.medicine} onChange={(e) => setStockFilter((prev) => ({ ...prev, medicine: e.target.value }))} placeholder="Filtrar por medicamento" className="pl-10 inp min-w-[220px]" />
                </div>
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const matchingUnit = units.find((unit) => unit.name.toLowerCase() === value.trim().toLowerCase());
                      setStockFilter((prev) => ({ ...prev, unit: value, unitId: matchingUnit?.id || "" }));
                    }}
                    onBlur={() => {
                      const matchingUnit = units.find((unit) => unit.name.toLowerCase() === stockFilter.unit.trim().toLowerCase());
            ...
            {loading ? (
              <div className="mt-6 text-slate-500">Carregando estoque...</div>
            ) : (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredStock.map((item) => (
                      <tr key={`${item.unitId}-${item.medicineId}`} className="border-t border-slate-100">
                        <td className="px-4 py-3 font-semibold text-[#1D3557]">{item.medicineName || item.medicineId}</td>
                        <td className="px-4 py-3 text-slate-600">{item.unitName}</td>
                        <td className={`px-4 py-3 font-semibold ${item.quantity <= 0 ? "text-amber-600" : "text-emerald-600"}`}>{item.quantity}</td>
                      </tr>
                    ))}
                    {filteredStock.length === 0 && (
                      <tr>
                        <td colSpan={3} className="px-4 py-10 text-center text-slate-400">Nenhum saldo encontrado para os filtros selecionados.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <div className="sc-card p-6">
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="text-xl font-bold text-[#1D3557]">Entradas e saídas</h2>
              </div>
              <div className="flex flex-col gap-2 md:flex-row md:items-center">
                <button type="button" onClick={exportMovementsCsv} className="flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-[#1D3557] transition hover:bg-slate-50">
                  <Download className="h-4 w-4" />
                  Exportar CSV
                </button>
                <select value={historyFilterMode} onChange={(e) => { setHistoryFilterMode(e.target.value); setHistoryFilterValue(""); setHistoryDateFrom(""); setHistoryDateTo(""); }} className="inp h-10 min-w-[180px] bg-white">
                ...
                {historyFilterMode === "type" ? (
                  <select value={historyFilterValue} onChange={(e) => setHistoryFilterValue(e.target.value)} className="inp h-10 min-w-[140px] bg-white">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <input type="date" value={historyDateFrom} onChange={(e) => handleHistoryDateFromChange(e.target.value)} className="inp min-w-[140px]" />
                    <input type="date" value={historyDateTo} onChange={(e) => handleHistoryDateToChange(e.target.value)} className="inp min-w-[140px]" />
                  </div>
                ) : (
                  <input value={historyFilterValue} onChange={(e) => setHistoryFilterValue(e.target.value)} placeholder={historyFilterMode === "user" ? "Nome do usuário" : historyFilterMode === "unit" ? "Nome da unidade" : historyFilterMode === "medicine" ? "Nome do medicamento" : "Filtrar pelo texto"} className="inp min-w-[240px]" />
                )}
              </div>
            </div>
                ...
                <tbody>
                  {filteredMovements.map((item) => (
                    <tr key={item.id} className="border-t border-slate-100 align-top">
                      <td className="px-4 py-3 font-mono-nums text-xs text-slate-500">{formatDateTime(item.createdAt)}</td>
                      <td className="px-4 py-3">
                        <span className={`rounded px-2 py-1 text-xs font-semibold ${item.type === "ENTRY" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                  ...
                  {filteredMovements.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-4 py-10 text-center text-slate-400">Nenhuma movimentação encontrada.</td>
                    </tr>
                  )}
                </tbody>
              </table>
